# -*- coding: utf-8 -*-
# @File    : SausageMain.py
# @Time    : 2020/3/23 16:54
# @Author  : Moeyu522 Lee
# @Describe: 项目主入口
# 详细说明：
# 项目名称： 基于CNN的香肠剔除系统研究
# 项目涉及主要技术：Opencv、颜色检测、卷积神经网络、PyQt、Python、51单片机
import _thread
import sys
import threading
import time
...
class SausageMain(threading.Thread):
    ...
    def run(self):
        while True:
            self.ui_class.bad_number.setText("test")
            # 是否接收单片机消息
            if self.control_flag is True:
                self.trigger_control = self.ui_class.serial.receive_data()
                if self.trigger_control == 't':
                    self.todo()
            else:
                self.todo()
    ...
    def todo(self):
        ...
        self.image_ndarray = self.ui_class.camera.getImage(delay_time=self.next_pic_time)
        # 将图像转换成BGR的, 在图像的处理过程中需要转换成BGR的(除CNN识别时为RGB)
        self.image_ndarray = cv.cvtColor(self.image_ndarray, cv.COLOR_RGB2BGR)
        # 原图像的w, h , c 信息
        (image_h, image_w, _) = self.image_ndarray.shape
        # opencv处理照片
        ...
        # 计算下一次拍照的时间
        self.next_pic_time = Image_UTIL.Util.getTimeFromPosition(self.object_bottom_value_invalid_max)

        # 遍历image_info中的cut得到每一个的预测结果
    ...
    def getInfoAndSend(self, passageway_number, passageway_info):
        ...
        for pos in bottom_value[0]:
            send_time.append(Image_UTIL.Util.getTimeFromPosition(pos))
        ...
        for _index, image in enumerate(cut[0]):
            re = self.ui_class.cnn_model.PredictFromImageNdarray(
                image_ndarray=Image_UTIL.Util.fillAndResizeImage(image))
            if re != 2:
                # 坏的香肠
...
if __name__ == '__main__':
    ...
    app = QApplication([])
    widget = QWidget()
    ui = Image_UI.UIDesign.UiMainForm()
    ui.setupUi(widget)
    widget.show()
    sys.exit(app.exec_())
